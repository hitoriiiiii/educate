    ...
    <div class="historyNotification">
        ...
        <div class="content">
            ...
            <!-- <div class="abnormalInfo">
                <div class="subTitle">患者异常</div>
                <div class="subContent" v-for="(item, index) in hisAbnormalInfo.patientAbnormal" :key="item.id" :value="item.id">
            ...
            <div class="abnormalInfo">
                ...
                <div class="subContent" v-for="(item, index) in hisAbnormalInfo" :key="item.id" :value="item.id">
                    {{item.name}} - {{item.area}} - {{item.sex}} - {{item.age}}
                    <span :style="{color: '#D11212'}">{{item.type}}</span>
                </div>
            </div>
...
const hisAbnormalInfo = ref([])

// 调用api获取“历史异常提醒”
const getHistoryNotice = (flag = false) => {
    getNoticeApi(flag).then(res => {
        if(res.data.code === 200){
            const leng = ref(null)
            if(res.data.data.rows.length<=6){
                leng.value = res.data.data.rows.length  //如果历史异常小于6条，则展示全部数据
            }else{
                leng.value = 6 //如果历史异常大于6条，则展示6条，即最多展示6条数据
            }
            for(let i=0; i< leng.value; i++) {
                hisAbnormalInfo.value.push(
                    {
