...
import NurseInfo from './AccountSteps/HeadnurseInfo.vue';
import NursePassword from './AccountSteps/HeadnursePassword.vue';
...
onMounted(() => {
  //获取激活选项卡的值
  const savedActive = localStorage.getItem('handActiveTab');
...
const saveActive = (key) => {
  activeKey.value = key;
  localStorage.setItem('handActiveTab', key);
};

// 监听页面刷新事件
window.addEventListener('beforeunload', () => {
  localStorage.setItem('handActiveTab', activeKey.value);
});
</script>
