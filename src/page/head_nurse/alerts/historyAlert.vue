    ...
    <div class="alertLayout">
        ...
        <div class="searchBox">
            <div class="keywordsSearch">
                姓名：<a-input v-model:value="search.name" placeholder="请输入姓名" allowClear :style="{width: '400px'}"></a-input>
            </div>
            <div class="allotBtn">
                <a-button type="primary" @click="getHistortList">查询</a-button>
            </div>
        </div>
        <!-- 异常提醒表格 -->
...
import {handHistoryAlert} from '@/api/head_nurse/handAlerts'

const search = reactive({ // 高级检索的选项
    name: '',
})
// 表格页码相关设置
...
const getHistortList = () =>{
    ...
    // 调用 api 获取所有随访任务分配的列表，设置dataSource的值
    handHistoryAlert(search.name).then(res =>{
        if(res.data.code === 200){
            console.log('历史',search.name)
