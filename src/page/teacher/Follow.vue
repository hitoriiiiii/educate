...
<template>
    <div class="contentHeader">
        ...
        <span class="headerDetail" style="margin-right: 10px;">本周评价计划</span>
        ...
        <a-form class="searchForm">
            ...
            <a-form-item label="状态" style="margin-right: 20px; width: 250px;">
                <a-select placeholder="请选择状态" v-model:value="serchInfo.statu" allowClear="true">
                    ...
                    <a-select-option value="2">已评价</a-select-option>
                    <a-select-option value="3">未评价</a-select-option>
                    <a-select-option value="4">评价异常</a-select-option>
                </a-select>
            </a-form-item>
            <a-form-item>
                ...
                <template #bodyCell="{ column, record }">
                    <template v-if="column.key === 'todeadline'">
                      <!-- 当本行的数据时负数时，显示“已逾期，且颜色是红色”；若数据为正数，显示获得的数据 -->
                        <span v-if="record[column.key] < 0" style="color: red">需评价</span>
                        <span v-else>{{ record[column.key] }}</span>
                    </template>
                    <template v-if="column.key === 'operation'">
                        <!-- 当状态为已评价时，操作栏的“去评价”变成灰色，且不能点击；否则“去评价”为绿色，且点击可以去评价 -->
                        <span v-if="record['todeadline'] === null" style="color: grey">去评价</span>
                        <!-- <span v-else>
                            <router-link 
                                :to="{ name: 'followDetail', params: { source: 'follow' } }" 
                                style="color: #c6cdfd;font-weight:500">去评价
                            </router-link>
                        </span> -->
